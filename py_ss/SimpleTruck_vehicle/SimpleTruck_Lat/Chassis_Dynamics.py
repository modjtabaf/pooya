...
class LongitudinalForce(blocks.Submodel):
    def __init__(self, iports, oport):
        super().__init__('LongitudinalForce', iports, oport)

        # nodes
        tire_info, road_slope_pct, F_aero, chassis_info = N(self._iports)
        F_lon_info, = N(self._oports)
        F_roll = 'F_roll'
        Fx = 'Fx'

        # blocks
        with self:
            blocks.BusSelector('', iport=tire_info, oports=[F_roll, Fx])
