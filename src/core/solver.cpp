...
#define CALC_K(K, t, X) \
    callback(t, X); \
    static Values K(model, states_info); \
    K.invalidate(); \
    K.set_states(h * X.derivs());

void rk4(const Model& model, SolverCallback callback, double t0, double t1, StatesInfo& states_info, double& new_h)
{
    double h = new_h = t1 - t0;

    // X0
    static Values X0(model, states_info);
    X0.invalidate();
    ...
    // X1 = X0 + K1/2
    static Values X1(model, states_info);
    X1.invalidate();
    ...
    // X2 = X0 + K2/2
    static Values X2(model, states_info);
    X2.invalidate();
    ...
    // X3 = X0 + K3
    static Values X3(model, states_info);
    X3.invalidate();
    ...
    static Values new_values(model, states_info);
...
void rkf45(const Model& model, SolverCallback callback, double t0, double t1, StatesInfo& states_info, double& new_h)
{
    double h = t1 - t0;
    ...
    static Values X4(model, states_info);
    X4.invalidate();
    ...
    static Values X5(model, states_info);
    X5.invalidate();
    ...
    static Values YT(model, states_info);
    YT.invalidate();
    ...
    static Values ZT(model, states_info);
    ZT.invalidate();
...
void euler(const Model& model, SolverCallback callback, double t0, double t1, StatesInfo& states_info, double& new_h)
{
    double h = new_h = t1 - t0;

    // ret = x0 + (t1 - t0)*callback(t0, x0)
    static Values values(model, states_info);
    values.invalidate();
